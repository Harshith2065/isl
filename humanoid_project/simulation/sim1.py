"""Humanoid walking environment version1 using PyBullet and Gymnasium for an input image."""
...
if __name__ == "__main__":
    # Ask user for image path to get initial pose
    img_path = input("Enter the path to your image for initial pose: ").strip()

    skeletons, _ = extract_keypoints(img_path)
    if len(skeletons) == 0:
        print("No people detected in the image. Exiting...")
        exit()

    # Select first skeleton (or implement largest bounding box if multiple)
    selected_skeleton = skeletons[0]
    theta_init = skeleton_to_joint_angles(selected_skeleton)

    env = HumanoidWalkEnv(render=True)
    obs, _ = env.reset(initial_pose=theta_init)
